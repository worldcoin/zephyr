...
BUILD_ASSERT(ASYNC_UART_STATUS_TIMEOUT != DMA_STATUS_FULL, "ASYNC_UART_STATUS_TIMEOUT must be different from DMA_STATUS_FULL");
			       ...
			       uint32_t channel, int status)
{
	...
	if (data->rx_next_buffer != NULL && status == DMA_STATUS_FULL) {
		async_evt_rx_buf_release(data);
	...
	} else if (status == DMA_STATUS_FULL) {
